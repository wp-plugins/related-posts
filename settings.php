...
function wp_rp_settings_scripts() {
	wp_enqueue_script('wp_rp_themes_script', plugins_url('static/js/themes.js', __FILE__),
						array('jquery'), WP_RP_VERSION);
	wp_enqueue_script("wp_rp_dashboard_script", plugins_url('static/js/dashboard.js', __FILE__),
						array('jquery'), WP_RP_VERSION);
	wp_enqueue_script("wp_rp_extras_script", plugins_url('static/js/extras.js', __FILE__),
						array('jquery'), WP_RP_VERSION);
}
...
function wp_rp_settings_admin_menu() {
	...
	if($count) {
		$title .= '<span class="update-plugins count-' . $count . '">';
		$title .= '	<span class="plugin-count">' . $count . '</span>';
		$title .= '</span>';
	}
...
add_action('wp_ajax_rp_subscribe', 'wp_rp_ajax_subscribe_callback');
...
function wp_rp_handle_postdata() {
	...
	if (!isset($_POST['_wp_rp_nonce']) || !wp_verify_nonce($_POST['_wp_rp_nonce'],
															'wp_rp_settings') ) {
...
function wp_rp_settings_page() {
	...
	$input_hidden = array(
		...
		'wp_rp_json_url' => esc_attr(WP_RP_STATIC_BASE_URL . WP_RP_STATIC_JSON_PATH),
